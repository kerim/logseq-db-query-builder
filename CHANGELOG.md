...
### Added
- **"deadline / scheduled" filter** (`deadline-scheduled`). Filters blocks by whether `:logseq.property/deadline` and/or `:logseq.property/scheduled` are present at all. Two dropdowns: which field (deadline or scheduled / deadline / scheduled) and which state (is not set / is set). Pair it with the task filter to find tasks with no dates on them. Block-only.
...
### Fixed
- Fix journal-date dropdowns stacking vertically instead of side-by-side: removed `width: 100%` from `.date-mode-content` so the flex item shrinks to content width
...
### Fixed
- **CRITICAL BUG**: Property filters were showing two value inputs instead of one
- Fixed 'value' input rendering to skip for property filters
- Property filters now correctly show only the type-specific input (dropdown, date picker, etc.)
- Removed duplicate "Enter value..." and "Property value..." inputs

### Technical
- Modified 'value' case in renderInputs() to check filter.type === 'property'
- Property filters now exclusively use renderPropertyValueInput() for value input
